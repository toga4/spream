...
// ModType represents the type of modification in a change record.
type ModType string

// Modification types.
const (
	ModType_INSERT ModType = "INSERT"
	ModType_UPDATE ModType = "UPDATE"
	ModType_DELETE ModType = "DELETE"
)

// HeartbeatRecord represents a heartbeat from the change stream.
type HeartbeatRecord struct {
	Timestamp time.Time `spanner:"timestamp" json:"timestamp"`
}

// ChildPartitionsRecord represents a child partitions notification from the change stream.
...
// ChildPartition represents a child partition in the change stream.
...
// Internal types for Spanner unmarshaling.
...
func decodeColumnTypeJSONToType(columnType spanner.NullJSON) Type {
	if !columnType.Valid {
		return Type{Code: TypeCode_NONE}
	}
	m, ok := columnType.Value.(map[string]any)
	if !ok {
		return Type{Code: TypeCode_NONE}
	}
	// "code" is a required field in the Spanner change stream type JSON.
	// Intentionally panic on assertion failure to surface schema violations immediately.
	...
	if aet, ok := m["array_element_type"].(map[string]any); ok {
		elem := Type{Code: TypeCode(aet["code"].(string))} // same as above
