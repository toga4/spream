...
func TestSubscriber_ShutdownDuringInitialize(t *testing.T) {
	storage := &mockPartitionStorage{
		getUnfinishedMinWatermarkPartitionFunc: func(ctx context.Context) (*PartitionMetadata, error) {
			// Block until the context is canceled, simulating a long-running query.
			<-ctx.Done()
			return nil, ctx.Err()
		},
	}
	...
	// Give Subscribe time to enter initialize.
	time.Sleep(50 * time.Millisecond)

	go func() {
		_ = subscriber.Shutdown(context.Background())
	}()

	select {
	case err := <-runDone:
		if diff := cmp.Diff(ErrShutdown, err, cmpopts.EquateErrors()); diff != "" {
			t.Errorf("Subscribe() error mismatch (-want +got):\n%s", diff)
		}
	case <-time.After(500 * time.Millisecond):
		t.Error("Subscribe() should return ErrShutdown when Shutdown is called during initialize")
	}
}

func TestSubscriber_CloseDuringInitialize(t *testing.T) {
	storage := &mockPartitionStorage{
		getUnfinishedMinWatermarkPartitionFunc: func(ctx context.Context) (*PartitionMetadata, error) {
			<-ctx.Done()
			return nil, ctx.Err()
	...
	case <-time.After(500 * time.Millisecond):
		t.Error("Subscribe() should return ErrClosed when Close is called during initialize")
	}
}
...
func TestSubscriber_fail(t *testing.T) {
	t.Run("first error is recorded", func(t *testing.T) {
		...
		p := s.err.Load()
		if p == nil || !errors.Is(*p, err1) {
			t.Errorf("err = %v, want %v", p, err1)
		}
	})
...
func TestSubscriber_BaseContextCancellation(t *testing.T) {
	t.Run("base context cancellation stops subscriber", func(t *testing.T) {
		...
		select {
		case err := <-runDone:
			// Base context cancellation returns context.Canceled to distinguish
			// from normal completion (nil).
...
func TestSubscriber_ShutdownTimeoutThenClose(t *testing.T) {
	baseCtx, baseCancel := context.WithCancelCause(context.Background())
	defer baseCancel(nil)
	...
	// Create a reader whose goroutine blocks on flushCtx.
	// This simulates a Consumer that is still processing during shutdown.
	flushCtx, flushCancel := context.WithCancel(context.Background())
	tracker := newInflightTracker(10)
	reader := &partitionReader{
		partitionToken: "test-reader",
	...
	// Shutdown with a short timeout should return DeadlineExceeded.
	...
	// Close should succeed: it cancels flushCtx, unblocking the goroutine.
	if err := s.Close(); err != nil {
		t.Errorf("Close() = %v, want nil", err)
	}
}

func TestSubscriber_startPartitionReaderRejectsDuringDrain(t *testing.T) {
	storage := &mockPartitionStorage{}
	s := newTestSubscriber(storage, 100*time.Millisecond)

	// Start drain so that draining flag is set.
	s.drain()
	...
	// startPartitionReader should be rejected because draining is true.
	s.startPartitionReader(partition)

	s.readersMu.RLock()
	_, exists := s.readers[partition.PartitionToken]
	s.readersMu.RUnlock()

	if exists {
		t.Error("startPartitionReader should not add a reader after drain has started")
	}
}
...
func TestNewSubscriber_Validation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cfg: &Config{
				SpannerClient:    new(spanner.Client),
				PartitionStorage: &mockPartitionStorage{},
				Consumer:         ConsumerFunc(func(ctx context.Context, record *DataChangeRecord) error { return nil }),
			},
			wantErr: "spream: StreamName is required",
		},
		{
			...
			wantErr: "spream: PartitionStorage is required",
		},
		{
			name: "nil Consumer",
			cfg: &Config{
			...
			wantErr: "spream: Consumer is required",
		},
	}
...
// #3: fail() and exitError() must be safe for concurrent use.

func TestSubscriber_ConcurrentFailAndExitError(t *testing.T) {
	ctx, cancel := context.WithCancelCause(context.Background())
	defer cancel(nil)
	...
	// Simulate multiple partition readers reporting errors concurrently.
	for i := range 100 {
		wg.Add(1)
		go func() {
			defer wg.Done()
			s.fail(fmt.Errorf("error %d", i))
		}()
	}

	// Simulate Subscribe() calling exitError().
	for range 100 {
		wg.Add(1)
		go func() {
			defer wg.Done()
...
// #7: Passing negative values to NewSubscriber causes panic in semaphore.NewWeighted
// or time.NewTicker. Without validation, a panic occurs here.

func TestNewSubscriber_NegativeValidation(t *testing.T) {
	dummyClient := new(spanner.Client)
	dummyStorage := &mockPartitionStorage{}
	dummyConsumer := ConsumerFunc(func(_ context.Context, _ *DataChangeRecord) error { return nil })
	...
	tests := []struct {
	...
	}{
		{
			...
			cfg: &Config{
				SpannerClient:    dummyClient,
				StreamName:       "test",
				PartitionStorage: dummyStorage,
				Consumer:         dummyConsumer,
				MaxInflight:      -1,
			},
			wantErr: "spream: MaxInflight must be non-negative",
		},
		{
			...
			wantErr: "spream: HeartbeatInterval must be non-negative",
		},
		{
			name: "negative PartitionDiscoveryInterval",
			cfg: &Config{
				SpannerClient:              dummyClient,
				StreamName:                 "test",
				PartitionStorage:           dummyStorage,
				Consumer:                   dummyConsumer,
				PartitionDiscoveryInterval: -1,
			},
			wantErr: "spream: PartitionDiscoveryInterval must be non-negative",
		},
	}
